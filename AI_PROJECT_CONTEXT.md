...
## 最近一次 UI/3D 修复记录（2026-03）

- `Scene3D` 的 `CameraController` 已拆分为“模式/角度切换时重定位”与“自动旋转状态同步”两条逻辑，避免 `autoRotate` 变化时重新套用相机预设，从而修复“暂停旋转导致视角重置”。
- `simulation` 模式现在也挂载 `OrbitControls`，并使用独立相机初始位姿（面向远处城市场景），支持拖拽旋转、缩放、平移。
- `EyeModel` 已重构为更清晰的分层结构（巩膜、视网膜、晶状体、虹膜、瞳孔、角膜、剖面辅助高光），并移除随机血管线与自动摆动动画，减少视觉噪声与“模型混乱”感。
- 调试建议更新：若再次出现“相机跳变”，优先检查 `Scene3D.tsx` 中 `CameraController` 的 effect 依赖，确保 `autoRotate` 不参与相机重定位 effect。
